...
var1 = open("/storage/emulated/0//records.txt", "r").read()
print(var1)
...
def cric_scores():  # Function to get cricket records and save in excel file
    ...
    workbook.save(str(file_name) + " - " + gender + "_" + Match_type + ".xlsx")
...
if len(indvl) > 0:  # if the records are separate for ODI, T20 and Tests
    ...
    # print(split)
    if split[2] == "Women\'s":  # if 3rd character is women's then it's women's record
        gender = "women"
        print(gender)
        if split[3] == "Twenty20":
            Match_type = "T20"
            ...
            Match_type = "Test"
            print(Match_type)
    else:  # if 3rd character is not women, then it's a men record
        gender = "men"
        print(gender)
        if split[2] == "Twenty20":
            Match_type = "T20"
